    ...
    public partial class ReadingRoomTwo : UserControl
    {
        ...
        private void RecvEnviroumentData(object Connection)
        {
            ...
            while (true)
            {
                ...
                */
                ...
                //修改数据库
                SQLString = string.Format("update tb_roomenviroument set room_temp = \"{0}\", room_hum = \"{1}\", room_light = \"{2}\",room_noise = \"{3}\" where room_name = \"{4}\";", Temp, Hum, Light, Noise, "阅片室2");
                frm_Main.DataBase.UpdateTable(SQLString);
        ...
        private void RecvClientCommand(object Connection)
        {
            ...
            while (true)
            {
                ...
                frm_Main.Log.WriteLog("接收到阅片室2客户端指令：" + Command);
