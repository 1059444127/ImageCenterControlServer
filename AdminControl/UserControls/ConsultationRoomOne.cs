...
namespace AdminControl
{
    ...
    public partial class ConsultationRoomOne : UserControl
    {
        ...
        public void RecvDeviceStatusThreadStart(Socket Connection)
        {
            ...
            string SQLString = string.Format("update tb_clientinformation set client_ip = \"{1}\", client_status = \"{0}\"  where client_name = \"{2}\";", ControlSocket.RemoteEndPoint.ToString().Split(':')[0], "Online", "会诊室1控制器");
        ...
        private void RecvDeviceStatus(object Connection)
        {
            ...
            while (true)
            {
                ...
                catch (Exception)
                {
                    ...
                    string SQLString = string.Format("update tb_clientinformation set client_ip = \"{1}\", client_status = \"{0}\"  where client_name = \"{2}\";",  ControlSocket.RemoteEndPoint.ToString().Split(':')[0], "Offline", "会诊室1控制器");
                    frm_Main.DataBase.UpdateTable(SQLString);
        ...
        public void RecvClientCommandThreadStart(Socket Connection)
        {
            ...
            string SQLString = string.Format("update tb_clientinformation set client_ip = \"{1}\", client_status = \"{0}\"  where client_name = \"{2}\";", ClientSocket.RemoteEndPoint.ToString().Split(':')[0], "Online", "会诊室1客户端");
            frm_Main.DataBase.UpdateTable(SQLString);
        ...
        private void RecvClientCommand(object Connection)
        {
            ...
            while (true)
            {
                ...
                catch (Exception)
                {
                    ...
                    string SQLString = string.Format("update tb_clientinformation set client_ip = \"{1}\", client_status = \"{0}\"  where client_name = \"{2}\";", ClientSocket.RemoteEndPoint.ToString().Split(':')[0], "Offline", "会诊室1客户端");
                    frm_Main.DataBase.UpdateTable(SQLString);
